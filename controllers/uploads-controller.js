...
const fileToUpload = async (req, res = response) => {
  ...
  if (!validTables.includes(table)) {
    return res.status(404).json({
      ok: false,
      message:
        'The table is not correct, it must be one of the following hospitals/doctors/users',
    });
  }
...
const returnsImage = (req, res = response) => {
  ...
  if (fs.existsSync(pathImage)) {
    ...
    const noImageFound = path.join(__dirname, `../uploads/${table}/no-img.jpg`);
    res.sendFile(noImageFound);
    console.log(noImageFound);
  }
};
